...
- [ ] **Email notifications (Resend) — IN PROGRESS, planned 2026-07-24.** Decisions locked: **weekly Monday briefing** of upcoming shows; **all active staff** emailed on a new show; **@mention** emails; **two opt-out toggles** (event emails / briefing) on Profile; **send from a verified own domain**. Architecture: event emails via a server route triggered off notification inserts; weekly digest via a scheduled route. Blocked on Jacky's setup: Resend account + domain DNS verification, Resend API key + Supabase service-role key (server-only Vercel env vars), confirm public app URL. Then: migration for email-pref columns + settings UI. *(doc-approval workflow, direct chat still backlog; Leave may be re-enabled.)*
