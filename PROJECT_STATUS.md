...
- 2026-07-28 — **SOP items seeded twice** ("double data"). Check-then-insert seeding raced: two concurrent first-opens of a show (two tabs / refresh mid-load / prefetch vs navigation) both saw an empty checklist and both inserted all 26 template rows. Fixed in v16 (dedupe + unique index on `show_id,section,title`) and the seed is now an `upsert … ignoreDuplicates`. **Can't fully automate** — no static check catches a read-then-write race; class lesson: **any seed/insert that's guarded only by "does it exist yet?" needs a DB uniqueness constraint**, because two requests can pass the check at once. Audit other seed paths against this.
