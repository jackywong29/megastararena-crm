...
- **Broadcasts** (2026-07-27, schema-v15) — `/dashboard/broadcasts`, **admin + department heads only** (`canSendBroadcasts`, nav gated in sidebar + mobile). Manual team email: pick an audience (all staff / by department / by role), write subject + body, then **"Open in mail app"** builds a `mailto:` BCC batch and opens the sender's own email (Gmail/Outlook) with everything prefilled — they review and hit send. Also "Save as draft" and "Copy emails". No email API/credentials — v1 delivery is the sender's own mail app. `broadcasts` table stores draft/sent history (subject, body, audience, recipient_count, status). Modeled on Clancy's broadcasts. *Phase-2 option: automated one-click send via a Gmail app password (nodemailer) — same table/UI.*
...
- [ ] **Run `schema-v15.sql`** in Supabase (`broadcasts` table). Until run, the Broadcasts tab errors for admins/dept-heads. After running, tell Claude → `npm run db:schema`.
